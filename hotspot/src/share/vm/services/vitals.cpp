...
#include "services/vitals_internals.hpp"
...
void inc_classes_loaded(size_t count) {
  Atomic::add_ptr(count, (jlong*)&g_classes_loaded);
}

void inc_classes_unloaded(size_t count) {
  Atomic::add_ptr(count, (jlong*)&g_classes_unloaded);
}

void inc_threads_created(size_t count) {
  Atomic::add(count, (jlong*)&g_threads_created);
}
...
public:
  ...
  void print_table(outputStream* st, const print_info_t* pi, print_context_t* pc,
                   const record_t* values_now = NULL) const {
    ...
    if (values_now != NULL) {
      // if now sample is given, include it into the calculation of the column width
      update_widths_from_one_record(values_now, NULL, g_widths, pi);
    }
    ...
    if (values_now != NULL && pi->reverse_ordering == false) {
      print_one_record(st, values_now, NULL, g_widths, pi);
      st->cr();
    }
...
public:
  ...
  void print_all(outputStream* st, const print_info_t* pi, const record_t* values_now = NULL) const {
    ...
    if (pi->max == 0 || pc.records_printed < pi->max) {
      ...
      _short_term_table->print_table(st, pi, &pc, values_now);
      st->cr();
    }
...
public:
  ...
  virtual void run() {
    ...
    for (;;) {
      take_sample();
      ::sleep(RecordTables::the_tables()->short_term_interval());
...
void sample_jvm_values(record_t* record, bool avoid_locking) {
  ...
  // Metaspace
  set_value_in_record(g_col_metaspace_committed, record, MetaspaceAux::committed_bytes());
  set_value_in_record(g_col_metaspace_used, record, MetaspaceAux::used_bytes());

  if (Metaspace::using_class_space()) {
    set_value_in_record(g_col_classspace_committed, record, MetaspaceAux::committed_bytes(Metaspace::ClassType));
    set_value_in_record(g_col_classspace_used, record, MetaspaceAux::used_bytes(Metaspace::ClassType));
  }
...
void print_report(outputStream* st, const print_info_t* pinfo) {
  ...
  if (info.sample_now) {
    values_now = (record_t*) os::malloc(record_size_in_bytes(), mtInternal);
    values_now->timestamp = 0; // aka Now
    sample_values(values_now, true /* never lock for now sample - be safe */ );
  }

  RecordTables::the_tables()->print_all(st, &info, values_now);
